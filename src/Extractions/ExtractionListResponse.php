<?php

declare(strict_types=1);

namespace XTwitterScraper\Extractions;

use XTwitterScraper\Core\Attributes\Optional;
use XTwitterScraper\Core\Attributes\Required;
use XTwitterScraper\Core\Concerns\SdkModel;
use XTwitterScraper\Core\Contracts\BaseModel;

/**
 * @phpstan-import-type ExtractionJobShape from \XTwitterScraper\Extractions\ExtractionJob
 *
 * @phpstan-type ExtractionListResponseShape = array{
 *   extractions: list<ExtractionJob|ExtractionJobShape>,
 *   hasMore: bool,
 *   nextCursor?: string|null,
 * }
 */
final class ExtractionListResponse implements BaseModel
{
    /** @use SdkModel<ExtractionListResponseShape> */
    use SdkModel;

    /** @var list<ExtractionJob> $extractions */
    #[Required(list: ExtractionJob::class)]
    public array $extractions;

    #[Required]
    public bool $hasMore;

    #[Optional]
    public ?string $nextCursor;

    /**
     * `new ExtractionListResponse()` is missing required properties by the API.
     *
     * To enforce required parameters use
     * ```
     * ExtractionListResponse::with(extractions: ..., hasMore: ...)
     * ```
     *
     * Otherwise ensure the following setters are called
     *
     * ```
     * (new ExtractionListResponse)->withExtractions(...)->withHasMore(...)
     * ```
     */
    public function __construct()
    {
        $this->initialize();
    }

    /**
     * Construct an instance from the required parameters.
     *
     * You must use named parameters to construct any parameters with a default value.
     *
     * @param list<ExtractionJob|ExtractionJobShape> $extractions
     */
    public static function with(
        array $extractions,
        bool $hasMore,
        ?string $nextCursor = null
    ): self {
        $self = new self;

        $self['extractions'] = $extractions;
        $self['hasMore'] = $hasMore;

        null !== $nextCursor && $self['nextCursor'] = $nextCursor;

        return $self;
    }

    /**
     * @param list<ExtractionJob|ExtractionJobShape> $extractions
     */
    public function withExtractions(array $extractions): self
    {
        $self = clone $this;
        $self['extractions'] = $extractions;

        return $self;
    }

    public function withHasMore(bool $hasMore): self
    {
        $self = clone $this;
        $self['hasMore'] = $hasMore;

        return $self;
    }

    public function withNextCursor(string $nextCursor): self
    {
        $self = clone $this;
        $self['nextCursor'] = $nextCursor;

        return $self;
    }
}
